...
Comparative study of classical and DFT modelisation of $\text{TiO}_2$ nanoparticles
...
We then ran a single-point energy calculation on each conformation using traditional Kohn-Sham (KS) DFT at the PBE/DZVP level of theory, using a plane wave cutoff of 2000$\,$Ry using \textsc{CP2K}'s Quickstep module for DFT calculations\cite{quickstep}.
...
\tab Seeing as core electrons have been shown to have a negligible impact on the interatomic interactions in $\text{TiO}_2$\cite{comp_exptl3,electronic_structure} (especially at $p=0\,$GPa), we used norm-conserving pseudopotentials developed by Goedecker, Teter, and Hutter\cite{gth} to avoid having to explicitly account for them in our electronic density optimisation routine, thereby making our \textit{ab-initio} calculations significantly computationally cheaper.
