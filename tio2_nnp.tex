...
\label{pes}
\caption{Schematic representation of the fault in our approach. Here both PESs are assumed to be 1-dimensional (instead of depending on the $3N$ position variables of the system's $N$ atoms). We can see how conformations (black dots) sampled near the minimum of the MA PES (blue curve) are much more spread out in energy when projected on the PES obtained using DFT (red curve), yielding a similar comparison to the one drawn by our data.}
\end{figure*}
